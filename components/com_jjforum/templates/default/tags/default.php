...
<h1>view post by tag: <?php echo $this->tag; ?></h1>
<?php
	if($this->user->id == 0){ ?>
	<div class="jjForum_user0">
	    <?php echo JText::_('Please');?> <a href="#" class="jjForum_login"><?php echo JText::_('login');?></a> / <a href="#" id="jjForum_register"><?php echo JText::_('register');?></a> <?php echo JText::_('in order to post');?> 
...
<?php
	...
	<div class="jjForum_user1">
        ...
        <img style="float:left; margin:0px 10px 10px 0" width="50" src="https://graph.facebook.com/<?php echo substr($this->fbuser, 7); ?>/picture">
        <?php } else {?>
        <img style="float:left; margin:0px 10px 10px 0" width="50" src="http://www.gravatar.com/avatar/<?php echo md5( strtolower( trim($this->user->email))); ?>.png"  />
        <?php } ?>
...
<?php
    ...
    <div id="JJordering">
        ...
        <select onchange="location.href=this.options[this.selectedIndex].value;" id="ordering" name="orderPosts">
            <option <?php echo ($this->order == 'default')?'selected="selected"':'';?> value="<?php echo JRoute::_('index.php?option=com_jjforum&view=tags&tag='.$this->tag.'&Itemid='.$this->itemid.'&limitstart=0&order=default')?>"><?php echo JText::_('default');?></option>
            <option <?php echo ($this->order == 'newest')?'selected="selected"':'';?> value="<?php echo JRoute::_('index.php?option=com_jjforum&view=tags&tag='.$this->tag.'&Itemid='.$this->itemid.'&limitstart=0&order=newest')?>"><?php echo JText::_('Newest first');?></option>
            <option <?php echo ($this->order == 'oldest')?'selected="selected"':'';?> value="<?php echo JRoute::_('index.php?option=com_jjforum&view=tags&tag='.$this->tag.'&Itemid='.$this->itemid.'&limitstart=0&order=oldest')?>"><?php echo JText::_('Oldest first');?></option>
            <option <?php echo ($this->order == 'rating')?'selected="selected"':'';?> value="<?php echo JRoute::_('index.php?option=com_jjforum&view=tags&tag='.$this->tag.'&Itemid='.$this->itemid.'&limitstart=0&order=rating')?>"><?php echo JText::_('By rating');?></option>
        </select>
    </div>
    <div class="JJclear"></div>
	<div id="jjForum_post">    
	...
	foreach($this->item->data as $post){
		...
		?>
			...
			<h3 class="JJToggler">
				...
				<a id="<?php echo $post->id;?>-<?php echo $post->catid;?>-<?php echo $this->itemid;?>" name="post<?php echo $post->id;?>" rev="" class="JJtitle" href="#"><?php echo $post->title;?></a>
				<?php endif; ?>
			...
			<div class="JJpostTxt" style="display:none">
				<div class="JJpostSide">
					<?php if($this->params->get('show_editor') != '0'):?>
					<span class="JJuserBy">
						<img width="50" src="http://www.gravatar.com/avatar/<?php echo md5( strtolower( trim($post->email))); ?>.png"  />
				...
				<div class="JJpostMain">
					...
					<?php } ?></p>
					<?php endif;?>        
                    Published in <a href="index.php?option=com_jjforum&view=posts&id=<?php echo $post->catid;?>&Itemid=<?php echo $this->itemid; ?>"><?php echo $post->category;?></a> >>
...
<div class="JJpagination">
	<?php echo ($this->item != '0')?$this->item->paging:''; ?>
</div>
